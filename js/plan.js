...
function pRows(d){
  ...
  segs.forEach((s, i) => {
    const prev = segs[i - 1];
    if(prev && toMin(s.a) - toMin(prev.b) >= PSTEP) out.push({gap:toMin(s.a) - toMin(prev.b), a:prev.b});
    out.push({s});
  });
  return out;
}
...
function renderPlan(){
  ...
  const rows = items.map((it, i) => {
    ...
    return it.gap ? pGapRow(it, pos) : pRow(d, it.s, pos);
  }).join('');
  ...
  $('#view').innerHTML = `<div class="pwrap">
    ...
    <div class="ptpl">
      ...
      <button class="lblbtn" id="pSave" title="Ghi các khối của ngày này trở lại mẫu">⤓ Lưu vào mẫu</button>
    </div>
    ...
    <div class="plist">
      ...
      ${rows}</div>
    <button class="lblbtn" id="pAdd">+ Thêm khối</button>
  ...
  $('#pAdd').onclick = () => {
    ...
    const a = last && toMin(last.b) < PDAY - PSTEP ? last.b : '12:00';
    const s = {id:uid(), a, b:pHM(Math.min(PDAY, toMin(a) + 60)), k:'rest', n:'Khối mới'};
